...
const std::string VulkanBase::getAssetPath() const
{
	return "../data/";
}
...
VkPhysicalDeviceFeatures VulkanBase::createDeviceFeatures()
{
	VkPhysicalDeviceFeatures deviceFeatures{};
...
VkSurfaceFormatKHR VulkanBase::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
	...
	if (availableFormats.size()) {
		...
		throw std::runtime_error("swap surface format requested, but not available!");
	}
}
...
void VulkanBase::pickPhysicalDevice()
{
	...
	window->setTitle(window->getTitle() + " - " + deviceProperties.deviceName);
}
